...
async function initSettingsPage(user) {
    ...
    // 1. منطق التنقل الجانبي (Side Tabs) بكلاسات فريدة
    // ------------------------------------------------
    ...
    // 2. إدارة حالة الهاتف (التحقق اليدوي 0000)
    // ------------------------------------------------
    ...
    if (btnStartVerify) {
        btnStartVerify.onclick = (e) => {
            ...
            document.getElementById('otpSentMessage').textContent = `كود التحقق مرسل للرقم: ${currentNum}`;
        ...
        btnConfirmOTP.onclick = async (e) => {
            ...
            if (otpInput.value === '0000') {
                try {
                    ...
                    if (res.ok) {
                        alert('✅ تم توثيق رقم الهاتف بنجاح');
    ...
    // 3. إدارة وضع التعديل (Edit Mode) - الإصلاح الجذري
    // ------------------------------------------------
    function toggleEditMode(enable) {
        // فك قفل كافة الحقول في الفورم بالكامل (حتى في التبويبات المخفية)
        ...
        // زر التحقق يبقى ظاهراً إذا لم يكن الرقم موثقاً
    ...
    // 4. تعبئة البيانات والمصفوفات (منع التكرار)
    // ------------------------------------------------
    function populateFields() {
        ...
        // إفراغ الحاويات لمنع تكرار البيانات
    ...
    // 6. القائمة الكاملة للدول والمدن
    // ------------------------------------------------
    const arabCountries = {
        ...
        [t('js-country-algeria')]: [t('js-city-algiers'), t('js-city-oran'), t('js-city-constantine')],
        [t('js-country-tunisia')]: [t('js-city-tunis'), t('js-city-sfax'), t('js-city-sousse')],
        [t('js-country-egypt')]: [t('js-city-cairo'), t('js-city-alexandria')],
        [t('js-country-saudi')]: [t('js-city-riyadh'), t('js-city-jeddah'), t('js-city-mecca')],
    ...
    function initLocation(currentLoc) {
        ...
        Object.keys(arabCountries).forEach(c => {
            ...
            if (c === initialCountry) opt.selected = true;
        ...
        const fillCities = (country, city) => {
            ...
            if (country && arabCountries[country]) {
                arabCountries[country].forEach(cn => {
                    const opt = new Option(cn, cn);
                    if (cn === city) opt.selected = true;
